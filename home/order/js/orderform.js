 ...
 * 2017-10-09	Webサイトリニューアルに伴いソース整理
 */
...
$(function () {
	...
	*/
	...
	$('#category_selector').change(function () {
		...
		$.ajax({
			...
			data: {
				'act': 'itemlist',
				'categoryid': $.itemparam.categoryid
			}
		}).done(function (res) {
			...
			$.msgbox("Error: " + error + "<br>アイテム情報が取得できませんでした。");
		});
	...
	$('#step2').on('change', '.size_table tbody tr:not(".heading") td[class*="size_"] input', function () {
		...
		$(this).closest('.size_table').next('.btmline').find('.cur_amount').text(amount.toLocaleString('ja-JP'));
	...
	*/
	...
	$('#estimation_wrap').on('change', 'tbody input[type="number"]', function () {
		$.check_NaN(this, '1');
		var self = $(this);
		var args = self.attr('class').split('_');
		var amount = self.val() - 0;
		var cost = self.closest('td').prev('td').text() - 0;
		var fee = cost * amount;
		self.closest('td').next('td').children('p:eq(0)').text(fee.toLocaleString('ja-JP'));
		$.ajax({
			url: '/php_libs/orders.php',
			async: false,
			type: 'POST',
			...
			$.msgbox("Error: " + error + "<br>枚数の変更ができませんでした。");
		});
	});
	...
	*
	$('#nodeliday').change( function(){
		...
		$.ajax({
			url:'/php_libs/orders.php', type:'post', dataType:'json', async:true, timeout:5000, data:postData
		}).done(function(r){
			if(r.options.length==0) return;
			if(val==1){
				// 納期指定なしの場合は納期を削除
	...
	*/
	...
	$('#deliverytime').change(function () {
		...
		$.ajax({
			...
			$.msgbox("Error: " + error + "<br>お届け時間の指定ができませんでした。");
		});
	});
	...
	$('#note_design, #note_printcolor').focusout(function () {
		var textName = {
			'note_design': 'デザインのご要望',
			'note_printcolor': 'デザインの色のご要望'
		};
		var key = $(this).attr('name');
		var val = $(this).val();
		var postData = {
		...
		$.ajax({
			...
			$.msgbox("Error: " + error + "<br>" + textName[key] + "の更新ができませんでした。");
		});
	});
	...
	*/
	...
	$(':radio[name="publish"]', '#step5').change(function () {
		...
		$.ajax({
			...
			$.msgbox("Error: " + error + "<br>デザイン掲載のチェックが更新できませんでした。");
		});
	});
		...
		$.ajax({
			...
			if (r.length != 0) {
				if (typeof r.id == 'undefined') {
					...
					$('#signin_name').text(r.customername + ' 様');
					$('#mypage_button').text('マイページ');
					$('#signout').prop('hidden', false);
					$('#cart_total').text($('#estimation_wrap .total').text());
					$('#cart_amount').text($('#estimation_wrap .totamount').text());
					$.setCustomer2();
				}
			}
		}).fail(function (xhr, status, error) {
...
//				uploadTable += '<tr class="template-download fade">';
//				uploadTable += '<td><span class="preview">
//				if (file.thumbnailUrl) {
//					uploadTable += '<img src="'+file.thumbnailUrl+'">';
//				} 
//				uploadTable += '</span></td>';
//				uploadTable += '<td><p class = "name" ><span >'+file.name+'</span></p>';
//				uploadTable += '<span class = "path" hidden >'+file.url+'</span>';
//				if (file.error) {
//					uploadTable += '<div><span class="label label-danger">Error</span>'+file.error+'</div>';
//				} else {
//					uploadTable += '<div><span class="label" style="font-size:1.2rem;font-weight:bold;color:#0275d8;">完了</span></div>';
//				}
//				uploadTable += '</td>';
//				uploadTable += '<td><span class="size">'+o.formatFileSize(file.size)+'</span></td>';
//				uploadTable += '<td>';
//				if (file.deleteUrl) {
//					uploadTable += '<button class = "btn btn-danger delete" data-type="'+file.deleteType+'" data-url="'+file.deleteUrl+'"';
//					if (file.deleteWithCredentials) {
//						uploadTable += 'data-xhr-fields='{"withCredentials":true}'';
//					}
//					uploadTable += '>';
//					uploadTable += '<i class = "fa fa-trash" aria-hidden="true"></i>';
//					uploadTable += '<span>削除</span></button>';
//				} else {
//					uploadTable += '<button class="btn btn-warning cancel">';
//					uploadTable += '<i class="fa fa-ban" aria-hidden="true"></i>';
//					uploadTable += '<span>キャンセル</span></button>';
//				}
//				uploadTable += '</td></tr>';
//			}
			...
			$('#datepicker').datepicker({
				...
				onSelect: function (dateText, inst) {
					...
					$.ajax({
						...
						$.msgbox("Error: " + error + "<br>ご希望納期の更新ができませんでした。");
					});
				}
			});
		...
		setCustomer: function (args) {
			...
			if (args) {
				...
				$.ajax({
					...
					alert("Error: " + error + "<br>顧客情報の更新ができませんでした。");
				});
			}
		},
		setCustomer2: function (args) {
			...
			$.ajax({
				...
				alert("Error: " + error + "<br>顧客情報が取得できませんでした。");
			});
		},
		login: function (fd) {
			/*
		...
		getPage: function (itemid, colorcode) {
			...
			$.ajax({
				...
				alert("Error: " + error + "<br>注文情報の更新ができませんでした。");
			});
				...
				for (var i = 0; i < r.category.length; i++) {
					...
					*/
					...
					for (var sizeid in c['size']) {
						...
						dat += '<td class="ar"><p>' + sub.toLocaleString('ja-JP') + '</p><span class="btn_sub del_cart" onclick="$.deleteitem(this,' + c.categoryid + ',' + c.itemid + ',\'' + c.colorcode + '\',' + sizeid + ');">削除</span></td></tr>';
						/*dat += '<td class="ac"></td>';*/
					}
				}
			...
			$.getJSON($.TLA.api + '?callback=?', {
				...
				$.each(r, function (key, val) {
					...
					if (key == 0) {
						...
						size_body = '<th>' + val['cost'].toLocaleString('ja-JP') + ' 円</th><td class="size_' + val['id'] + '_' + val['name'] + '_' + val['cost'] + '"><input id="size_' + val['id'] + '" type="number" value="' + amount + '" min="0" max="999" class="forNum" onfocus="$.focusNumber(this);" onblur="$.blurNumber(this);"/></td>';
				...
				pane.find('.cur_amount').text(sum.toLocaleString('ja-JP'));
		...
		showPrintPosition: function () {
			...
			$.ajax({
				...
				alert("Error: " + error + "<br>プリント位置情報が取得できませんでした。");
			});
		...
		updatePosition: function () {
			...
			$.ajax({
				...
				alert("Error: " + error + "<br>プリント情報の更新ができませんでした。");
			});
		...
		updateOptions: function (r) {
			...
			$('#estimation_wrap .base').text(base.toLocaleString('ja-JP'));
			$('#estimation_wrap .tax').text(tax.toLocaleString('ja-JP'));
			$('#estimation_wrap .credit').text(credit.toLocaleString('ja-JP'));
			$('#estimation_wrap .total').text(total.toLocaleString('ja-JP'));
			$('#estimation_wrap .perone').text(perone.toLocaleString('ja-JP'));
			...
			$('#estimation_wrap .discountfee').text(r.options.discount.toLocaleString('ja-JP'));
			$('#estimation_wrap .package').text(r.options.packfee.toLocaleString('ja-JP'));
			$('#estimation_wrap .expressfee').text(r.options.expressfee.toLocaleString('ja-JP'));
			...
			//			$('tr.total td span', '#showcart_box').text(total.toLocaleString('ja-JP'));
			//			$('tr:first td span', '#floatingbox, #showcart_box').text(r.options.amount.toLocaleString('ja-JP'));
			//			$('tr:last td span', '#floatingbox').text(perone.toLocaleString('ja-JP'));
		},
		updateUser: function () {
			/*
			 *	ユーザー情報の登録
		...
		confirm: function () {
			...
			if ($('#pass').is(':visible')) {
				...
				$.ajax({
					...
					alert("Error: " + error + "<br>会員情報の確認ができませんでした。");
				});
				if (isExistUser) {
					return;
				}
			...
			$.ajax({
				...
				for (var i = 0; i < items.length; i++) {
					...
					for (var j = 0; j < items[i]['size'].length; j++) {
						...
						cost += '<p>' + items[i]['size'][j]['cost'].toLocaleString('ja-JP') + '</p>';
						vol += '<p>' + items[i]['size'][j]['amount'] + '<span>枚</span></p>';
						sub += '<p>' + items[i]['size'][j]['subtotal'].toLocaleString('ja-JP') + '<span>円</span></p>';
				...
				var option = r.option;
				tr += '<tr><td colspan="3" class="ac">小計</td><td class="ar">' + amount.toLocaleString('ja-JP') + '<span>枚</span></td><td class="ar">' + item_price.toLocaleString('ja-JP') + '<span>円</span></td></tr>';
				tr += '<tr><td colspan="4">プリント代</td><td class="ar">' + option.printprice.toLocaleString('ja-JP') + '<span>円</span></td></tr>';
				...
				for (var m in option) {
					...
					if (m == 'discount') {
						...
						tr += '<tr><td colspan="4">' + optname[m] + '<span id="spec_discount">' + spec + '</span></td><td class="ar fontred">▲<ins id="conf_discount">' + option[m].toLocaleString('ja-JP') + '<ins><span>円</span></td>';
					} else if (m == 'carriage') {
						...
						tr += '<tr><td colspan="4">' + optname[m] + '<span>（' + pack + '）</span></td><td class="ar"><ins id="conf_pack">' + option[m].toLocaleString('ja-JP') + '</ind><span>円</span></td>';
					} else if (m == 'expressfee' && option['expressInfo'] != '') {
						tr += '<tr><td colspan="4">' + optname[m] + '<span>（' + option['expressInfo'] + '）</span></td><td class="ar"><ins id="conf_expressfee">' + option[m].toLocaleString('ja-JP') + '</ind><span>円</span></td>';
					}
				}
				...
				$('#conf_item tfoot .base ins').text(base.toLocaleString('ja-JP'));
				$('#conf_item tfoot .tax ins').text(tax.toLocaleString('ja-JP'));
				$('#conf_item tfoot .credit ins').text(credit.toLocaleString('ja-JP'));
				$('#conf_item tfoot .tot ins').text(total.toLocaleString('ja-JP'));
				$('#conf_item tfoot .per ins').text(perone.toLocaleString('ja-JP'));
				...
				alert("Error: " + error + "<br>注文情報が取得できませんでした。");
			});

			if (isError) {
				$txt = "【重要なお知らせ】<br>";
				$txt += "平素はタカハマライフアートをご利用いただき、誠にありがとうございます。<br>";
				$txt += "現在、システム障害が発生しておりお申し込みフォームがご利用出来ない状態になっております。<br>";
				$txt += "皆さまにはご不便をおかけいたしまして誠に恐縮ではございますが、下記よりご連絡を頂けますでしょうか。<br><br>";
				$txt += "info@takahama428.com<br>";
				$txt += "タカハマライフアートサポートチーム";
				$.msgbox($txt);
				//				$.updateItem(["","","",""]);
				$.back(0);
		...
		digits: function ($target, args) {
			...
			var intervalID = setInterval(function () { // 1ミリ秒ごとに実行
				...
				if (cnt >= limit) {
					...
					$target.text(args.toLocaleString('ja-JP'));
				}
			}, 1);
		},
