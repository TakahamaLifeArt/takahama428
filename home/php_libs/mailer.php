 ...
 */
		...
		try{
			...
			if($result){
				...
				if($result && $this->info['title']=='request'){
					...
					$addr = htmlspecialchars($this->info['addr0'], ENT_QUOTES, "utf-8");
					$addr .= htmlspecialchars($this->info['addr1'], ENT_QUOTES, "utf-8");
					if(!empty($this->info['addr2'])){
						$addr = $addr.' '.htmlspecialchars($this->info['addr2'], ENT_QUOTES, "utf-8");
					}
					$args = array(
						"requester"=>htmlspecialchars($this->info['customername'], ENT_QUOTES, "utf-8"),
						"subject"=>htmlspecialchars($this->info['subject'], ENT_QUOTES, "utf-8"),
						"message"=>htmlspecialchars($this->info['message'], ENT_QUOTES, "utf-8"),
	...
	*/
	protected function send_mail($mail_text, $name, $to, $attach, $mailuser='info', $add_info=array()){
		...
		$msg .= mb_convert_encoding($mail_text.$add_info[0],"JIS","UTF-8");	// ここで注文情報をエンコードして設定
...
//				$this->info['a2'] = $this->info['a2'];
//				$this->info['a4'] = $this->info['a4'];
//				$this->info['a8'] = $this->info['a8'];
//				$this->info['a9'] = $this->info['a9'];
//				$this->info['a10'] = $this->info['a10'];
//				$this->info['a11'] = $this->info['a11'];
//				$this->info['a12'] = $this->info['a12'];
//				$this->info['a13'] = $this->info['a13'];
				$conn = new ConnDB();
	...
	*/
	public function send_multi($mail_info, $subject, $sendto, $fromname, $formaddr, $bcc="", $cc="", $attach=""){
		...
		$mail_info .= mb_convert_encoding($footer,'JIS','UTF-8');
		$msg .= mb_convert_encoding($mail_info,"JIS","UTF-8");	// ここで本文をエンコードして設定
