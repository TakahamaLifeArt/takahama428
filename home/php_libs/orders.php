	...
	public function getItemlist($id){
		...
		if(isset($_ITEM_DATA)){
			for($a=0; $a<count($_ITEM_DATA); $a++){
				...
				$pageinfo = $this->itemPageInfo($itemcode, 'code');
	...
	*/
	public function reqConfirmation(){
		...
		for($i=0; $i<count($_SESSION['orders']['attach']); $i++){
			$option['attach'][] = $_SESSION['orders']['attach'][$i]['img']['name'];
		}
...
if(isset($_REQUEST['act'])){
	...
	case 'itemlist':
		$dat = $orders->getItemlist($_REQUEST['categoryid']);
		break;
